...
import { ListPricingTemplates200Response } from "../model/listPricingTemplates200Response";
...
export enum PricingTemplatesApiApiKeys {
  TilledApiKey,
}

export class PricingTemplatesApi {
  ...
  public setApiKey(key: PricingTemplatesApiApiKeys, value: string) {
    (this.authentications as any)[PricingTemplatesApiApiKeys[key]].apiKey =
      value;
  }
   ...
   * @param id
   */
  public async getPricingTemplate(
    tilled_account: string,
    id: string,
    options: { headers: { [name: string]: string } } = { headers: {} }
  ): Promise<{ response: http.IncomingMessage; body: PricingTemplate }> {
    const localVarPath =
      this.basePath +
      "/v1/pricing-templates/{id}".replace(
        "{" + "id" + "}",
    ...
    if (tilled_account === null || tilled_account === undefined) {
      throw new Error(
        "Required parameter tilled_account was null or undefined when calling getPricingTemplate."
      );
    }
    ...
    if (id === null || id === undefined) {
      throw new Error(
        "Required parameter id was null or undefined when calling getPricingTemplate."
      );
    }
        ...
        localVarRequest(localVarRequestOptions, (error, response, body) => {
          if (error) {
            ...
            if (
              ...
              body = ObjectSerializer.deserialize(body, "PricingTemplate");
   ...
   * @param tilled_account The id of the Tilled Account (usually starting with the prefix &#x60;acct_&#x60;) that the request is performed on behalf of.
   * @param status String indicating the status to filter the result by.
   * @param type String indicating the type to filter the result by.
   * @param merchant_account_id A merchant accountId using pricing templates associated with the provided partner accountId. If provided, the returned pricing templates will be ones that are applied to the merchant accountId provided, if any.
   * @param merchant_activation_status If a merchant accountId is provided, the query will return pricing templates associated with the merchant that have this activation status.
   ...
   */
  public async listPricingTemplates(
    tilled_account: string,
    status?: Array<"active" | "archived" | "pending">,
    type?: "card" | "ach_debit" | "eft_debit",
    merchant_account_id?: string,
    merchant_activation_status?: "active" | "pending" | "cancelled",
    offset?: number,
    limit?: number,
    options: { headers: { [name: string]: string } } = { headers: {} }
  ): Promise<{
    response: http.IncomingMessage;
    body: ListPricingTemplates200Response;
  }> {
    const localVarPath = this.basePath + "/v1/pricing-templates";
    let localVarQueryParameters: any = {};
    let localVarHeaderParams: any = (<any>Object).assign(
      {},
      this._defaultHeaders
    ...
    if (tilled_account === null || tilled_account === undefined) {
      throw new Error(
        "Required parameter tilled_account was null or undefined when calling listPricingTemplates."
      );
    }
    ...
    if (merchant_account_id !== undefined) {
      localVarQueryParameters["merchant_account_id"] =
        ObjectSerializer.serialize(merchant_account_id, "string");
    }

    if (merchant_activation_status !== undefined) {
      localVarQueryParameters["merchant_activation_status"] =
          ...
          if (error) {
            ...
            if (
              ...
              body = ObjectSerializer.deserialize(
                body,
                "ListPricingTemplates200Response"
              );
              resolve({ response: response, body: body });
            } else {
              reject(new HttpError(response, body, response.statusCode));
