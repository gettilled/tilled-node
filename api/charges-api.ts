...
export const ChargesApiAxiosParamCreator = function (
  ...
  return {
    ...
    getCharge: async (
      ...
      const localVarPath = `/v1/charges/{id}`.replace(
        `{${"id"}}`,
...
export const ChargesApiFp = function (configuration?: Configuration) {
  ...
  return {
    ...
    async getCharge(
      ...
      (axios?: AxiosInstance, basePath?: string) => AxiosPromise<Charge>
    > {
      const localVarAxiosArgs = await localVarAxiosParamCreator.getCharge(
        tilled_account,
        id,
        options
      );
...
export const ChargesApiFactory = function (
  ...
  return {
     ...
     */
    getCharge(
      requestParameters: ChargesApiGetChargeRequest,
      options?: AxiosRequestConfig
    ): AxiosPromise<Charge> {
      return localVarFp
        .getCharge(
