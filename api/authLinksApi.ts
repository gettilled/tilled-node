...
import { AuthLinkCreateParams } from "../model/authLinkCreateParams";
...
export enum AuthLinksApiApiKeys {
  TilledApiKey,
}

export class AuthLinksApi {
  ...
  public setApiKey(key: AuthLinksApiApiKeys, value: string) {
    (this.authentications as any)[AuthLinksApiApiKeys[key]].apiKey = value;
  }
   ...
   * @param AuthLinkCreateParams
   */
  public async createAuthLink(
    tilled_account: string,
    AuthLinkCreateParams: AuthLinkCreateParams,
    options: { headers: { [name: string]: string } } = { headers: {} }
  ): Promise<{ response: http.IncomingMessage; body: AuthLinkDto }> {
    const localVarPath = this.basePath + "/v1/auth-links";
    ...
    if (tilled_account === null || tilled_account === undefined) {
      throw new Error(
        "Required parameter tilled_account was null or undefined when calling createAuthLink."
      );
    }

    // verify required parameter 'AuthLinkCreateParams' is not null or undefined
    if (AuthLinkCreateParams === null || AuthLinkCreateParams === undefined) {
      throw new Error(
        "Required parameter AuthLinkCreateParams was null or undefined when calling createAuthLink."
      );
    }
    ...
    return interceptorPromise.then(() => {
      ...
      return new Promise<{ response: http.IncomingMessage; body: AuthLinkDto }>(
        (resolve, reject) => {
          localVarRequest(localVarRequestOptions, (error, response, body) => {
            if (error) {
              ...
              if (
                ...
                body = ObjectSerializer.deserialize(body, "AuthLinkDto");
